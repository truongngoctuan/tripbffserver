...
import {
  resolveImageUrlFromExternalStorageId,
...
export class TripQueryHandler {
  ...
  async GetById(
    ...
    return this.TripRepository.get(loggedUserId, id, createdById).then(
      (trip) => {
        if (!trip) return trip;

        if (trip.infographics && trip.infographics.length > 0) {
          const latestExportedInfographics = trip.infographics.filter(
            (item) => item.status === "EXPORTED"
          );

          if (latestExportedInfographics.length > 0)
            trip.latestExportedExternalStorageId =
              latestExportedInfographics[
                latestExportedInfographics.length - 1
              ].externalStorageId;
        }
  ...
  async list(ownerId: string): Promise<ITrip[]> {
    return this.TripRepository.list(ownerId).then((trips) => {
    ...
    trip.locations = trip.locations.map((location) => {
      location.images = location.images.map((image) => {
        if (image.externalStorageId) {
          image.externalUrl = resolveImageUrlFromExternalStorageId(
            image.externalStorageId
          );
          image.thumbnailExternalUrl = resolveThumbnailImageUrlFromExternalStorageId(
            image.externalStorageId
          );
        }
        return image;
