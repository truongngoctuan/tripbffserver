import {
  ITripsRepository,
  ITripMinimized,
} from "../../_core/models/ITripsRepository";
...
export class TripsRepository implements ITripsRepository {
  ...
  private async getPublicTrips(
    ...
    return await this._mg.UserTripsDocument.aggregate([
      {
        $match: {
  ...
  public async listNewsFeed(
    ...
    const publicTrips: ITripMinimized[] = userTrips.map((item: any) => {
